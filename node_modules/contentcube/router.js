...
module.exports = function(application) {
	...
	instance.route = function(request, callback) {
		...
		var controllerName = application.config.get('routing.defaultControllerName');
		var actionName = application.config.get('routing.defaultActionName');
		var moduleName = application.config.get('routing.defaultModuleName');
			...
			function byConvention(error, didRouteByConfiguration) { 
				...
				Step(
					function routeWithModule() {
						...
						if (segments.length > 0 && application.config.get('system.modulesEnabled', false)) {
							var lookupModuleName = segments[0];
							var modulePath = Path.join(application.config.get('system.localPath'), Path.normalize('/application/modules/' + lookupModuleName));
						...
						if (segments.length > 0) {
							...
							if (actionName.indexOf("-") != -1) {
								...
								nameChunks.forEach(function(chunk) {
									normalizedActionName += chunk.ucfirst();
								});
	...
	function matchConfigurationRoute(uri) {
		var routes = application.config.get('routing.routes', []);
		var sorted = []; 
