...
module.exports = function(request) {
	...
	instance.url = {
		...
		baseUrl: function() {
			var args = Array.prototype.slice.call(arguments);
			return global.application.getConfig('system.baseUrl') + path.join.apply(path, args);
		},
		build: function(route) {
			var uri = application.getConfig('system.baseUrl');
			
			if (application.getConfig('system.modulesEnabled')) {
				uri = path.join(uri, route.module || application.getConfig('routing.defaultModuleName'));
			}
			
			uri = path.join(uri, route.controller || application.getConfig('routing.defaultControllerName'));
			uri = path.join(uri, route.action || application.getConfig('routing.defaultActionName'));
	...
	instance.renderView = function(viewName, viewData) {
		...
		if (global.application.getConfig('system.modulesEnabled', false)) {
			pathToView = '/application/modules/' + request.moduleName + '/views/' + viewName;
		}
		
		var filepath = path.join(global.application.getConfig('system.localPath'), path.normalize(pathToView));
