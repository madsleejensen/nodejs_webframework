...
module.exports = function Application(localPath) {
	...
	function onHttpRequestReceived(request, response) {
		...
		Step(
			...
			function handleStaticContentRequest() {
				if (!instance.getConfig('system.serveStaticContent', false)) {
					return this();
				}
	...
	Step(
		function initialize() {
			mConfig = require(Path.join(localPath, "application/config/production"));
			...
			if (instance.getConfig('system.webSocketsEnabled', false)) {
				instance.socketIO = SocketIO.listen(instance.httpServer);
				//instance.socketIO.set('log', false);
			}
			
			instance.setConfig('system.localPath', localPath);
			this();
		},
