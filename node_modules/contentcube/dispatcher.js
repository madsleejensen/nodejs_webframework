...
module.exports = function(application) {
	...
	instance.dispatch = function(request, response, callback) {
		...
		(function loop(callback) {
			Step(
				function preDispatch() {
					application.emitter.emit("dispatch:before", [request, response], this);
				},
				...
				function postDispatch(error) {
					if (error) throw error;
					application.emitter.emit("dispatch:after", [request, response], this);
				},
				function onActionHandled(error) {	
					if (error) {
						if (error.family == 'contentcube' && request.controllerName != 'error') { // prevent it from going into a infinite loop.
							...
							request.moduleName = application.config.get('routing.defaultModuleName');
		...
		function loadController(callback) {
			...
			var filepath = Path.join(application.config.get('system.localPath'), Path.normalize(pathToController));
